#!/usr/bin/env bash
# Real-path verification for feat/prepayment-invoice-link (3 rounds).
...
DB="${OPENRENTAL_PREPAY_DB:-mr_bin_prepay_link}"; URL="postgresql://$(whoami)@localhost:5432/${DB}"
PORT="${OPENRENTAL_PREPAY_PORT:-3106}"; LOG="$(mktemp -t openrental_prepay.XXXXXX)"
...
OPENRENTAL_BASE_URL="http://localhost:${PORT}" OPENRENTAL_DEV_LOG="${LOG}" python3 -W ignore e2e/round_prepay_invoice_link.py

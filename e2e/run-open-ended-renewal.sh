...
DB_NAME="mr_bin_rolling_browser_${$}"
DB_URL="postgresql://$(whoami)@localhost:5432/${DB_NAME}"
PORT="${OPENRENTAL_PORT:-3115}"
LOG="$(mktemp -t openrental_rolling_browser.XXXXXX)"
SERVER_PID=""
...
if ! DATABASE_URL="${DB_URL}" OPENRENTAL_BASE_URL="http://localhost:${PORT}" \
  OPENRENTAL_DEV_LOG="${LOG}" OPENRENTAL_TEST_DB="${DB_NAME}" \
  python3 -W ignore e2e/open_ended_renewal_campaign.py; then
  tail -160 "${LOG}"
  exit 1
fi
...
echo "ROLLING RENEWAL DISPOSABLE RUN PASSED; cleanup will remove ${DB_NAME}"

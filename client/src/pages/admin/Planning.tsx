...
import { isCalendarDate, isPlanningWindowAvailable, shiftPlanningDate } from "@shared/planning";
...
export default function Planning() {
  ...
  const rows = baseRows.filter(row => filters.status === "all" || (filters.status === "available_window" ? isPlanningWindowAvailable(row.days) : filters.status === "available" ? row.days[focus]?.state === "available" : row.days[focus]?.overdue || row.days[focus]?.conflict || !["available", "reserved"].includes(row.days[focus]?.state)));
  const pageCount = Math.max(1, Math.ceil(rows.length / 50)), page = Math.min(pageIndex, pageCount - 1);
  const visibleRows = rows.slice(page * 50, (page + 1) * 50);
  useEffect(() => { setPageIndex(0); }, [filters, start, focus]);
  ...
  return <DashboardLayout><div className="planning-page">
    ...
    <section className="planning-controls" aria-label={t("planning.filters")}>
      ...
      <div className="planning-filterbar"><label className="planning-search"><span><Search size={15} />{t("planning.search")}</span><input value={filters.search} onChange={e => update({ search: e.target.value })} maxLength={100} /></label><label>{t("planning.category")}<select aria-label={t("planning.category")} value={filters.category} onChange={e => update({ category: e.target.value })}><option value="">{t("planning.all")}</option>{categories.map(c => <option key={c}>{c}</option>)}</select></label><label>{t("planning.location")}<select aria-label={t("planning.location")} value={filters.location} onChange={e => update({ location: e.target.value })}><option value="">{t("planning.all")}</option>{locations.map(c => <option key={c}>{c}</option>)}</select></label><label>{t("planning.show")}<select aria-label={t("planning.show")} value={filters.status} onChange={e => update({ status: e.target.value as PlanningFilters["status"] })}>{(["all", "available", "available_window", "attention"] as const).map(v => <option key={v} value={v}>{t(`planning.${v}`)}</option>)}</select></label></div>
